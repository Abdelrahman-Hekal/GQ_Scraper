...
def initialize_bot():
    ...
    prefs = {"profile.default_content_setting_values.geolocation": 1, "profile.managed_default_content_settings.images": 1, "profile.default_content_setting_values.notifications": 1}  
...
def scrape_articles(driver, driver_tr, output1, page, month, year):
    ...
    for _ in range(100):  
        try:
            ...
            date = wait(driver, 3).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "b[class='time']")))[-1].get_attribute('textContent').strip()
            if date.count('.') == 1:
                art_month = int(date.split('.')[0])
                art_year = year                    
            elif date.count('.') == 2:
            ...
            try:
                span = wait(driver, 3).until(EC.presence_of_element_located((By.CSS_SELECTOR, "span[class='load-more']")))
                url = wait(span, 3).until(EC.presence_of_element_located((By.TAG_NAME, "a"))).get_attribute('href')
                driver.get(url)
                time.sleep(3)
            except:
                break
    ...
    # scraping posts urls 
    try:
        posts = wait(driver, 2).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "aside[class='alriclebox']")))    
    except:
    ...
    for post in posts:
        try:
            try:
                date = wait(post, 2).until(EC.presence_of_element_located((By.CSS_SELECTOR, "b[class='time']"))).get_attribute('textContent').strip()
                if date.count('.') == 1:
                    art_month = int(date.split('.')[0])
                    art_year = year                    
                elif date.count('.') == 2:
    ...
    for i, link in enumerate(links):
        ...
        art_id = ''
        try:
            art_id = link.split('_')[-1].replace('.html', '')
        except:
            pass
        ...
        sig = ''       
        try:
            p = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "p[class='signature']")))
            sig = p.get_attribute('innerHTML').split('<span></span>')[0]
        except Exception as err:
            pass        
        ...
        try:
            p = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "p[class='signature']")))
            date = p.get_attribute('innerHTML').split('<span></span>')[-1].split('.')
        except Exception as err:
            pass
        ...
        title = ''             
        try:
            title = wait(driver, 4).until(EC.presence_of_element_located((By.TAG_NAME, "h1"))).get_attribute('textContent').strip()
        except:
        ...
        des = ''             
        try:
            des = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "section[class='content']"))).get_attribute('textContent').strip()
        except:
        ...
        row['articlesignature'] = sig
        row['articledatetime'] = date            
            
        # article category
        cat = ''             
        try:
            cat = wait(driver, 4).until(EC.presence_of_element_located((By.TAG_NAME, "h6"))).get_attribute('textContent').strip()
        except:
            pass 
        ...
        header = ''             
        try:
            header = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "p[class='p']"))).get_attribute('textContent').strip()
        except:
            pass
        ...
        imgs = ''
        try:
            div = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "section[class='content']")))
    ...
    if data.shape[0] > 0:
        ...
        writer = pd.ExcelWriter(output1, date_format='d/m/yyyy', engine_kwargs={'options':{'strings_to_urls': False}})
...
def get_inputs():
    ...
    path = os.getcwd()
    if '\\' in path:
        path += '\\GQ_settings.xlsx'
    else:
        path += '/GQ_settings.xlsx'

    if not os.path.isfile(path):
        print('Error: Missing the settings file "GQ_settings.xlsx"')
